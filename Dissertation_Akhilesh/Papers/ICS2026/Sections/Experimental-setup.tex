  ...
  The training data is collected by a simple script, which runs each
  ...
  \end{equation}
  where $\bar{f}_{\text{prog}}$ is the median progress rate, $\bar{P}$ is mean
  ...
  \begin{equation}
      \mathbf{r} = \bigl[r_{\text{pow}},\; r_{\text{prog}}\bigr]^{\top},
  \end{equation}
  ...
  \begin{align}
      ...
      r_{\text{prog}} &= \phantom{-}\left(2\,\frac{f'_{\text{prog}}}{f_{\text{prog}}^{\max}} - 1\right),
      \quad\text{(high progress} \Rightarrow +1\text{)}
  \end{align}
  with $P_{\min}$, $P_{\max}$, and $f_{\text{prog}}^{\max}$ computed per
  ...
  Progress       & $\bar{f}_{\text{prog}}$       & $[0,\,300]$ \\
  Power          & $\bar{P}$\,(W)                & $[0,\,165]$ \\
  ...
  Reward         & $[r_{\text{pow}},\,r_{\text{prog}}]$ & $[-1,+1]^2$ \\
  ...
  \begin{equation}
      ...
      \quad
      \phi(t) = \frac{f_{\text{prog}}(t)}{f_{\text{prog}}^{\max}} \in [0, 1],
      \label{eq:preference}
  \end{equation}
  where $f_{\text{prog}}(t)$ is the current progress rate. At the start of
